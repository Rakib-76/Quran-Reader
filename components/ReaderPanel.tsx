...
import { Play, Pause, Bookmark, MoreHorizontal, Loader2 } from 'lucide-react';
...
export default function ReaderPanel() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white overflow-hidden shadow-[-4px_0_24px_-4px_rgba(0,0,0,0.02)] z-10 relative">
      
      {/* Centered Header Area */}
      <div className="py-6 md:py-10 border-b border-border-soft flex flex-col items-center justify-center shrink-0 bg-white relative">
        <div className="absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-border-soft to-transparent"></div>
        <h1 className="text-2xl md:text-[32px] font-bold text-text-dark mb-3 font-sans tracking-tight">Surah {data.meta.englishName}</h1>
          ...
          {data.mergedAyahs.map((ayah, index) => {
            ...
            return (
              <React.Fragment key={ayah.numberInSurah}>
                <div className={`flex flex-col md:flex-row transition-all duration-500 ease-out p-5 md:p-6 -mx-2 md:-mx-6 rounded-[28px] ${isPlaying ? 'bg-primary-green/[0.04] shadow-[inset_0_0_0_1px_rgba(76,140,63,0.12)]' : 'group hover:bg-gray-50/60'}`}>
                  ...
                  <div className="flex flex-row md:flex-col items-center justify-between md:justify-start gap-4 md:gap-5 mb-5 md:mb-0 md:mr-8 shrink-0 md:py-2 w-full md:w-10">
                    ...
                    <div className={`flex flex-row md:flex-col gap-6 md:gap-4 transition-all duration-300 ${isPlaying ? 'opacity-100 translate-y-0' : 'opacity-100 md:opacity-0 md:group-hover:opacity-100 md:translate-y-1 md:group-hover:translate-y-0'}`}>
                      <button 
                        ...
                        className={`transition-all duration-300 ${isPlaying ? 'text-primary-green scale-110 drop-shadow-sm' : 'text-text-muted hover:text-primary-green hover:scale-110'}`} 
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-right pb-8 pt-1">
                      <p 
                        className="font-arabic font-medium antialiased text-text-dark optimize-legibility"
                        style={{ 
                    ...
                    {/* English Translation */}
                    <div className="text-left border-l-[3px] border-border-soft pl-5 py-1">
                      <span className="text-[11px] font-bold uppercase tracking-widest text-[#9BA89B] mb-2.5 block">
                        Sahih International
                      </span>
                      <p 
                        className="text-text-dark/90 leading-relaxed font-sans"
                        style={{
                ...
                {index !== data.mergedAyahs.length - 1 && (
                  <div className="h-px bg-border-soft/60 w-full" />
                )}
              </React.Fragment>
            );
          })}
